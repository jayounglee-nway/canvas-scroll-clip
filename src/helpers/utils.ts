 ...
 * List of events
 **/
export const EventList = Object.values(AppEvent)
  .map((e: Record<string, unknown>) => Object.values(e))
  .flat();
...
export async function getImage(imageLink: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () => {
      reject(
        new AppError(
          `Image with name '...${imageLink.slice(-20)}' was not found.`
        )
      );
    };
  });
...
export function preloadImages(
  ...
  const arrayOfImages = new Array(frameOptions.count)
    .fill(0)
    .map((_elem, index) => {
 ...
 * @returns number
 */
export function getScrollFraction(
  cViewport: ICanvasViewport,
  scrollTop = getScrollTop()
): number {
  console.log(scrollTop);
  console.log(
    "to",
    (scrollTop - cViewport.start - cViewport.top) /
      (cViewport.bottom - cViewport.start - cViewport.screen.height) <
      0
      ? "그대로"
      : "움직임"
  );
  // return (
  //   (scrollTop - start) /
  //   (cViewport.bottom - (last - start) - cViewport.screen.height)
  // );
  return (
    (scrollTop - cViewport.start - cViewport.top) /
    (cViewport.bottom - cViewport.start - cViewport.screen.height)
 ...
 * @returns {string}
 */
export function getFramePathByIndex(
  frameOptions: IFrame,
  frameNumber = 1
): string {
  return [
 ...
 * @throws {AppError}
 */
export function getImageStructure(
  ...
  if (frameCount.toString().length > seq.length) {
    throw new AppError(
      `Leading zeros in first frame path has to be more than the frame count and sequence at the end.`
    );
  }
...
export function getImageSequence(imageName: string): string {
  ...
  if (sequence.length < 2) {
    throw new AppError(
      'Bad image sequence format. Should start with 0 and be longer than 2 numbers, f.e. "frame_01.jpg"'
    );
  }
